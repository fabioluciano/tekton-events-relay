...
func TestCELIntegration_ExampleConfigParsing(t *testing.T) {
	// Verify that examples/config.toml parses successfully
	// This ensures CEL expressions in the example config are valid
	cfg, err := config.Load("../../examples/config.toml")
	if err != nil {
		t.Fatalf("failed to load examples/config.toml: %v", err)
	}
...
func TestCELIntegration_BuildActionHandlersFlow(t *testing.T) {
	...
	wrapped := wrapWithCEL(inner, cfg.Notifiers.GitHub.Actions.PRComment.When)
