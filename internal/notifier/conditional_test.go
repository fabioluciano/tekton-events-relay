...
func (m *mockActionHandler) Handle(ctx context.Context, e domain.Event) error {
	m.called = true
	return m.err
}
...
func TestConditionalHandler_NilProgram(t *testing.T) {
	...
	inner := &mockActionHandler{name: "test-handler", typ: ActionCommitStatus}
...
func TestConditionalHandler_CELFalse(t *testing.T) {
	...
	program, err := cel.Compile(`event.State == "failure"`)
	if err != nil {
		t.Fatalf("failed to compile CEL: %v", err)
	}
	...
	if entry.Message != "handler skipped by CEL condition" {
		t.Errorf("expected skip message, got %q", entry.Message)
	}

	// Verify log fields
	fields := entry.ContextMap()
	if fields["handler"] != "test-handler" {
	...
	if fields["event_run_name"] != "test-run" {
