...
// legacyNotifier implements the legacy Notifier interface (for adapter testing)
type legacyNotifier struct {
	name   string
	called int
}

func (l *legacyNotifier) Name() string { return l.name }
func (l *legacyNotifier) Notify(_ context.Context, _ domain.Event) error {
	l.called++
	return nil
}
...
func TestNotifierAdapter(t *testing.T) {
	// Legacy Notifier wrapped as ActionHandler
	legacy := &legacyNotifier{name: "legacy"}

	adapter := WrapNotifier(legacy)

	if adapter.Name() != "legacy" {
		t.Errorf("adapter name = %v, want legacy", adapter.Name())
	}
	if adapter.Type() != ActionCommitStatus {
		t.Errorf("adapter type = %v, want %v", adapter.Type(), ActionCommitStatus)
	}

	_ = adapter.Handle(context.Background(), domain.Event{})
	if legacy.called != 1 {
		t.Errorf("adapter called legacy %d times, want 1", legacy.called)
	}
}
